...
public class Creneau {
    ...
    private boolean verifSalle(){
		...
		for(int i = 0;i< activite.getSalles().size();i++) {
			if(activite.getSalles().get(i) == salle ) {
				estDedans = true;
			}
		}
		return estDedans;
    ...
    private boolean verifCapacite(){
    	int nbEleve = 0;
    	for(int i = 0;i< activite.getGroupes().size();i++) {
			nbEleve += activite.getGroupes().get(i).getEffectif();
		}
